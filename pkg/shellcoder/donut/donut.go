...
var log = logger.WithField("pkg", "donut")
...
func CreateModule(config *DonutConfig, inputFile *bytes.Buffer) error {
	...
	if config.Type == DONUT_MODULE_NET_DLL ||
		...
		if config.Verbose {
			log.Debug("Runtime:", config.Runtime)
		}
		...
		if config.Verbose {
			log.Debug("DLL function:", config.Method)
		}
		copy(mod.Method[:], []byte(config.Method))
	}
...
func CreateInstance(config *DonutConfig) (*bytes.Buffer, error) {
	...
	if config.InstType == DONUT_INSTANCE_PIC {
		if config.Verbose {
			log.Debugf("The size of module is %v bytes. Adding to size of instance.\n", modLen)
		}
		instLen += modLen
	}
	...
	if config.Entropy == DONUT_ENTROPY_DEFAULT {
		if config.Verbose {
			log.Debug("Generating random key for instance")
		}
		...
		if config.Verbose {
			log.Debug("Generating random key for module")
		}
		tk, err = GenerateRandomBytes(16)
		if err != nil {
			return nil, err
		...
		if config.Verbose {
			log.Debug("Generating random string to verify decryption")
		}
		sbsig := RandomString(DONUT_SIG_LEN)
		copy(inst.Sig[:], []byte(sbsig))

		if config.Verbose {
			log.Debug("Generating random IV for Maru hash")
		}
	...
	if config.Verbose {
		log.Debug("Generating hashes for API using IV:", inst.Iv)
	}
	...
	if config.Type == DONUT_MODULE_NET_DLL ||
		config.Type == DONUT_MODULE_NET_EXE {
		if config.Verbose {
			log.Debug("Copying GUID structures and DLL strings for loading .NET assemblies")
		}
		...
		if config.Verbose {
			log.Debug("Copying GUID structures and DLL strings for loading VBS/JS")
		}
	...
	if inst.Type == DONUT_INSTANCE_URL {
		if config.ModuleName != "" {
			if config.Entropy != DONUT_ENTROPY_NONE {
				...
				if config.Verbose {
					log.Debug("Generated random name for module :", config.ModuleName)
				}
			} else {
		...
		if config.Verbose {
			log.Debug("Setting URL parameters")
		}
		// append module name
		...
		copy(inst.Req[:], "GET")
		if config.Verbose {
			log.Debug("Payload will attempt download from:", string(inst.Url[:]))
		}
	}
	...
	if config.InstType == DONUT_INSTANCE_URL && config.Entropy == DONUT_ENTROPY_DEFAULT {
		if config.Verbose {
			log.Debug("encrypting module for download")
		}
