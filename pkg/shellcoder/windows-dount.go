...
import (
	...
	"github.com/Esonhugh/sliver-linux-tcp-stager-helper/pkg/shellcoder/donut"
	"github.com/sirupsen/logrus"
)

func GenerateWindowsX64ShellcodeFromBytes(payload []byte) ([]byte, error) {
	return DonutShellcodeFromPE(payload, "amd64", false, "", "", "", false, false, true)
}
...
func DonutShellcodeFromPE(pe []byte, arch string, dotnet bool, params string, className string, method string, isDLL bool, isUnicode bool, createNewThread bool) (data []byte, err error) {
	...
	config := donut.DonutConfig{
		...
		Thread:     thread,
		Verbose:    logrus.StandardLogger().Level >= logrus.DebugLevel,
	}
